...
import { HANDLE_CHANGE_POST_API_SAGA, GET_TASK_DETAIL_SAGA, GET_TASK_DETAIL, UPDATE_STATUS_TASK_SAGA, UPDATE_TASK_SAGA, CHANGE_TASK_MODAL, CHANGE_ASSIGNESS, REMOVE_USER_ASSIGN, ADD_COMMENT, DELETE_COMMENT, UPDATE_COMMENT } from '../../constants/Cyberbugs/TaskConstants'
...
function* insertCommentSaga(action) {
    ...
    try {
        ...
        //Cập nhật api status cho task hiện tại (Task đang mở modal)
        const { data, status } = yield call(() => taskService.insertComment(values));
        ...
        if (status == STATUS_CODE.SUCCESS) {
            ...
            //     taskId: taskUpdateStatus.taskId
            // })
            alert('Insert Comment successfully!')
        }
...
export function* theoDoiInsertCommentApi() {
    yield takeLatest(ADD_COMMENT, insertCommentSaga);
}
...
function* deleteCommentSaga(action) {
    ...
    try {
        ...
        //Cập nhật api status cho task hiện tại (Task đang mở modal)
        const { data, status } = yield call(() => taskService.deleteComment(idComment));
...
function* updateCommentSaga(action) {
    ...
    try {
        ...
        //Cập nhật api status cho task hiện tại (Task đang mở modal)
        const { data, status } = yield call(() => taskService.updateComment(values));
...
export function* theoDoiUpdateCommentApi() {
